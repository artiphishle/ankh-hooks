...
function isNumberArray(maybeNumberArray: unknown[]) {
  ...
  try {
    return maybeNumberArray.every(
      (maybeNumber) => !isNaN(maybeNumber as number) && !!(maybeNumber as number).toString().length,
    )
  } catch (error: any) {
    return false
  }
...
export function useColorValidator() {
  ...
  function isValidHex(colorValue: string) {
    ...
    const colorVal = (colorValue.startsWith("#") ? colorValue.slice(1) : colorValue).toLowerCase()
    return (colorVal.length === 3 || colorVal.length === 6) && !colorVal.match(/[^a-f0-9]/)
  }
